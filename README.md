...
현재 MVP는 앱 골격, 로컬 저장 흐름, 저장소 색인 기반 책 초안 생성을 검증하는 단계다. 실제 LM Studio 추론은 아직 연결하지 않았고, 생성기는 교체 가능한 service port 뒤의 deterministic scanner와 `esp-hal` 튜닝 profile로 동작한다.

## 주요 기능

- 책장: 최근 읽은 책, 상태 필터, cover grid, 모바일 하단 navigation
- 새 책 생성: repository URL, 모델, 독자 수준, 생성 깊이 입력
- Repository generation: Git checkout, 파일/문서 색인, profile 기반 draft book과 generation run 생성
...
```text
apps/
  api/      Fastify API, SQLite migration/seed, repository scanner/tutor services
...
- Repository scanner 기반 book generation and tutor persistence
